          ...
          {footerLinks.map((val)=>{
            return(
              <div key={val.key} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
                <h4 className={`font-poppins font-medium text-[18px] leading-[27px] text-white`}>{val.title}</h4>
                <ul className='list-none mt-4'>
                  {val.links.map((item, i)=>{
                    return(
                      <li className={`font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${i !== val.links.length - 1 ? "mb-4" : "mb-0"}`} key={item.name}>{item.name}</li>
                    )
                  })}
                </ul>
              </div>
